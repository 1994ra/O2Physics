...
/// \brief task for azimuthal correlation of heavy flavour hadron decay electron
/// with charge particles \author Rashi gupta <rashi.gupta@cern.ch>, IIT Indore
...
struct HfCorrelatorElHadrons {
  ...
  Configurable<float> etaTrackMinDcright{"etaTrackMinDcright", 0.22f,
                                         "Eta range for electron tracks"};
  ...
  Configurable<float> etaTrackMaxDcright{"etaTrackMaxDcright", 0.6f,
                                         "Eta range for electron Dcal tracks"};
  Configurable<float> etaTrackMaxDcleft{"etaTrackMaxDcleft", -0.22f,
                                        "Eta range for electron Dcal tracks"};
  Configurable<float> eopElMin{"eopElMin", 0.8f,
                               "Minimum E/p for electron tracks"};
  Configurable<float> eopElMax{"eopElMax", 1.2f,
                               "Maximum E/p for electron tracks"};
  ...
  Configurable<float> tpcnSigElMin{"tpcnSigElMin", -1.0f,
                                   "min Electron TPCnsigma"};
  Configurable<float> tpcnSigElMax{"tpcnSigElMax", 3.0f,
                                   "max Electron TPCnsigma"};
  ...
  HistogramConfigSpec hTrackInfoSpec{HistType::kTHnSparseD,
                                     {{500, 0, 160},
                                      ...
                                      {100, -1.5, 1.5},
                                      {100, 0, 7}}};
  HistogramConfigSpec hTrackallInfoSpec{HistType::kTHnSparseD,
                                        {{500, 0, 160},
                                         {300, -15, 15},
                                         {500, 0., 50.},
                                         {500, 0., 50.},
                                         ...
                                         {2000, -1, 1},
                                         {3, 0, 3}}};
  HistogramConfigSpec hClusterInfoSpec{HistType::kTHnSparseD,
                                       {{300, 0.0, 30.0},
                                        {100, -0.9, 0.9},
                                        {200, 0, 6.3},
                                        {50, 0, 50},
                                        {1800, -900, 900}}};
  HistogramConfigSpec hPIDSpec{HistType::kTHnSparseD,
                               {{500, 0.0, 50.0},
                                {500, 0., 50.},
                                {300, -15, 15},
                                {300, 0.0, 30.0},
                   ...
                   int SparseNo = -1)
  {
    ...
    for (const auto& aTrack : assotrks) {
      ...
      if (ptHad < ptAssocMin)
        continue;
      if (abs(aTrack.dcaXY()) > dcaXYTrackMax ||
          abs(eTrack.dcaZ()) > dcaZTrackMax)
        continue;
      if (!aTrack.isGlobalTrackWoDCA())
        continue;

      registry.fill(HIST("fSprshadroninformation"), aTrack.tpcSignal(),
                    aTrack.tpcNSigmaEl(), aTrack.p(), ptHad, etaHad, phiHad);

      if (pTcondition && (ptEle > ptHad))
        continue;

      Dphi = RecoDecay::constrainAngle(phiEle - phiHad,
                                       -o2::constants::math::PIHalf);
      Deta = etaEle - etaHad;
      if (SparseNo >= 0)
        hEHcorrArray[SparseNo]->Fill(ptEle, ptHad, Dphi, Deta);
    }
  }
               ...
               GTrks const& tracks)
  {
    ...
    for (auto& Track : tracks) {
      ...
      if (!Track.isGlobalTrackWoDCA())
        continue;
      PassEMCal = 0;
      if ((phiTrack > phiTrackMinEM && phiTrack < phiTrackMaxEM) &&
          (etaTrack > etaTrackMin && etaTrack < etaTrackMax))
        PassEMCal = 1; // EMcal acceptance passed
      if ((phiTrack > phiTrackMinDc && phiTrack < phiTrackMaxDC) &&
          ((etaTrack > etaTrackMinDcright && etaTrack < etaTrackMaxDcright) ||
           (etaTrack > etaTrackMinDcleft && etaTrack < etaTrackMaxDcleft)))
        PassEMCal = 2; // Dcal acceptance passed

      registry.fill(HIST("fSprsdEdxnSigmaPt"), Track.tpcSignal(),
                    tpcnsigmaTrack, pTrack, ptTrack, etaTrack, phiTrack,
                    dcaxyTrack, dcazTrack,
                    PassEMCal); // track infor after filter bit
      ...
      for (const auto& emtrack : tracksofcluster) {
        ...
        registry.fill(HIST("ClsTrkEtaPhiDiffTime"), deltaEtaEMT, deltaPhiEMT,
                      timeMatchCluster);

        // Track and cluster Matching
        if (TMath::Abs(deltaPhiEMT) > deltaphiMinEMC ||
            TMath::Abs(deltaEtaEMT) > deltaetaMinEMC)
          continue;
        if (timeMatchCluster > timeEMCClsCut)
          continue;

        if (fclusterinfo)
          registry.fill(HIST("fSprsClusterInfoAf"), EMatchCluster,
                        etaMatchCluster, phiMatchCluster, cluster.nCells(),
                        timeMatchCluster);

        registry.fill(HIST("fSprsPIDafterMatch"), mtrack.tpcSignal(),
                      tpcnsigmaMatchTrack, pMatchTrack, ptMatchTrack,
                      etaMatchTrack, phiMatchTrack);

        double eop = EMatchCluster / pMatchTrack;

        // Apply Electron Identification cuts

        if ((tpcnsigmaMatchTrack < tpcnSigElMin ||
             tpcnsigmaMatchTrack > tpcnSigElMax) ||
            (eop < eopElMin || eop > eopElMax) ||
            (m02MatchCluster < m02ElMin || m02MatchCluster > m02ElMax) ||
            (m20MatchCluster < m20ElMin || m20MatchCluster > m20ElMax))
          continue;

        registry.fill(HIST("fSprsPIDafterPIDcuts"), pMatchTrack, ptMatchTrack,
                      tpcnsigmaMatchTrack, EMatchCluster, m02MatchCluster,
                      m20MatchCluster);
